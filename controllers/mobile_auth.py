...
mobile_auth = Blueprint('mobile_auth', __name__, url_prefix='/mobile_auth')
...
def register():
    try:
        ...
        try:
            ...
            db.session.delete(new_user)  # Remove user if email sending fails
            db.session.commit()
            return jsonify({
                "status": "error",
...
@mobile_auth.route('/verify_register_otp', methods=['POST'])
...
@mobile_auth.route('/resend_register_otp', methods=['POST'])
...
@mobile_auth.route('/update-profile', methods=['PUT'])
@jwt_required()
def update_profile():
...
# Endpoint untuk meminta kode reset password
...
@mobile_auth.route('/verify-reset-otp', methods=['POST'])
...
@mobile_auth.route('/resend-reset-otp', methods=['POST'])
...
# Endpoint untuk mereset password dari dalam aplikasi (user sudah login)
@mobile_auth.route('/reset-password-loggedin', methods=['POST'])
@jwt_required()
...
@mobile_auth.route('/delete-user', methods=['DELETE'])
@jwt_required()
def delete_user():
    user_identity = get_jwt_identity()
    current_user = User.query.get(user_identity['id'])
...
@mobile_auth.route('/users', methods=['GET'])
def get_all_users():
    try:
...
@mobile_auth.route('/list-users', methods=['GET'])
def list_users():
    # Ambil token user dan verifikasi
    users = User.query.all()
    return render_template('list_users.html', users=users)

@mobile_auth.route('/edit-user/<int:id>', methods=['PUT'])
@jwt_required()
def edit_user(id):
    try:
